...
class HomeController extends Controller
{
    ...
    // //         ->where('status', AccountStatus::USING())
    // //         ->with('user_info')
    // //         ->first();
    // //     $postsByUser = Post::where('posts.created_by', $user['id'])
    // //         ->leftJoin('categories', 'categories.id', '=', 'posts.category_id')
    // //         ->select('posts.*', 'categories.name as nameCate', 'categories.slug as slugCate')
    // //         ->where('posts.deleted_at', null)
    // //         ->where('posts.status', BlogStatus::PUBLISHED())
    // //         ->get();
    // //     return view('home.author.index', compact('user', 'postsByUser'));
    // // }
}
